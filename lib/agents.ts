...
// --- Revision Agent (Enhanced with Web Search) ---
...
You have access to web search tools to fact-check legal authorities, verify current law, and ensure citation accuracy during the revision process.
...
Use web search to:
- Verify legal authorities and their current status
- Confirm case holdings and statutory text accuracy
- Check for recent changes in law or policy
- Validate procedural requirements and deadlines
- Cross-reference controversial or complex legal points
...
export const revisionAgent = new Agent({
  ...
  tools: [webSearchTool()], // Added web search tool for fact-checking
  outputType: PublicBenefitsReportData,
})
